...
echo '<div class="relative min-h-screen flex">';
   ...
   echo '<h1 class="text-xl font-semibold pb-10">Insert a Person</h1>';
      if (isset($_COOKIE["username"])) {
         
         echo '<form class="rounded-lg bg-blue-100 shadow-md px-8 pt-6 pb-8 mb-4" action="insert_person_sql.php" method=post>';
         ...
         if($result->num_rows != 0) {
            echo '<div class="mb-6">';
               ...
               echo '<input class="rounded py-2 px-2 w-full" type=text name="firstname">';
            echo '</div>';
            echo '<div class="mb-6">';
               echo '<label class="block text-gray-700 mb-2 font-medium" for="lastname">Last Name:</label>';
               echo '<input class="rounded py-2 px-2 w-full" type=text name="lastname">';
            echo '</div>';
            echo '<div class="mb-6">';
               echo '<label class="block text-gray-700 mb-2 font-medium" for="address">Address:</label>';
               echo '<input class="rounded py-2 px-2 w-full" type=text name="address">';
            echo '</div>';
            echo '<div class="mb-6">';
               echo '<label class="block text-gray-700 mb-2 font-medium" for="dob">Date of Birth:</label>';
               echo '<input class="rounded py-2 px-2 w-full" type=date name="dob">';
            echo '</div>';
            echo '<div class="mb-6">';
               echo '<label class="block text-gray-700 mb-2 font-medium" for="email">Email:</label>';
               echo '<input class="rounded py-2 px-2 w-full" type=text name="email">';
            echo '</div>';

            echo '<div class="mb-6">';
               echo '<label class="block text-gray-700 mb-2 font-medium" for="lname">Library Name:</label>';
               echo '<select class="rounded w-full py-2 px-2" name="lname">'; 
               while($val = $result->fetch_assoc()) {
                  echo "<option value='$val[lname]'>$val[lname]</option>"; 
            ...
            echo "<p class=\"text-xl font-semibold\">There are no libraries in the system! </p>"; 
         }
         echo "</form>";
      } 
      else {
