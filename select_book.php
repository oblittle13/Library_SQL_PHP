...
echo '<div class="relative min-h-screen flex">';
   ...
   echo '<div class="flex-1 m-8 p-10 bg-blue-50 shadow-lg rounded text-gray-800" id="content">';
      echo '<h1 class="text-xl font-semibold pb-10">Select a Book</h1>';
if(isset($_COOKIE["username"])){

   echo '<form class="bg-blue-100 shadow-md px-8 pt-6 pb-8 mb-4" action="select_book_sql.php" method=post>';
   ...
   $sql = "select isbn, title, author, lib_name from BOOK"; 
   $result = $conn->query($sql);
   if($result->num_rows != 0)
   {
      echo '<label class="block text-gray-700 mb-2 font-medium" for="isbn">Book title:</label>';
      // echo "Library Name: <select name=\"lname\">";
      echo '<select class="rounded w-full py-2 px-2" name="isbn">'; 
      while($val = $result->fetch_assoc())
      {
         echo "<option value='$val[isbn]'>$val[isbn] : $val[title] by $val[author] at $val[lib_name]</option>"; 
